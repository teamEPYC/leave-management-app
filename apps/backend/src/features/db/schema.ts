...
export type LeaveLimitType = (typeof leaveLimitTypeEnum.enumValues)[number];
...
export const LeaveTypeGroupTable = pgTable(
  ...
  (t) => [uniqueIndex("uq_leave_type_group").on(t.leaveTypeId, t.groupId)]
);

// ─────────────────────────────────────────────────────────────
// Leave Balances

export const LeaveBalanceTable = pgTable(
  "leave_balances",
  {
    id: uuid("id")
      .primaryKey()
    ...
    usedDays: numeric("used_days", { precision: 5, scale: 2 })
      .notNull()
      .default("0"),
    adjustedDays: numeric("adjusted_days", { precision: 5, scale: 2 }).default(
      "0"
    ),
  ...
  (t) => [
    uniqueIndex("uq_user_leave_type_period").on(
      t.userId,
      t.leaveTypeId,
      t.periodStart
    ),
...
export const LeaveBalanceAdjustmentTable = pgTable(
  ...
  (t) => [
    uniqueIndex("uq_leave_balance_adjustment").on(
      t.leaveBalanceId,
      t.createdAt
    ),
  ]
...
export const LeaveRequestTable = pgTable(
  ...
  (t) => [
    uniqueIndex("uq_user_leave_request").on(
      t.userId,
      t.leaveTypeId,
      t.startDate
    ),
...
export const HolidayTable = pgTable(
  "holidays",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (t) => [uniqueIndex("uq_holiday_date").on(t.organizationId, t.date)]
);
